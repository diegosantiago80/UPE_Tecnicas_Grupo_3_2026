...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_PersonaExistePorDni]') AND type in (N'P', N'PC'))
BEGIN
    EXEC sys.sp_executesql N'
    CREATE PROCEDURE SP_PersonaExistePorDni
        @Dni VARCHAR(20)
    AS
    BEGIN
        SET NOCOUNT ON;
        SELECT COUNT(1) FROM Persona WHERE DNI = @Dni;
    END'
END;
GO
    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            IF EXISTS (
                ...
                WHERE M.StockActual < D.Cantidad
            )
            BEGIN
                RAISERROR(''Stock insuficiente para completar la transaccion.'', 16, 1);
            END
...
-- setea EsEmpleado = 0 para un cliente que ya no es empleado de la farmacia
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_DesvincularEmpleadoComoCliente]') AND type in (N'P', N'PC'))
BEGIN
    EXEC sys.sp_executesql N'
    CREATE PROCEDURE SP_DesvincularEmpleadoComoCliente
        @IdCliente INT
    AS
    BEGIN
        SET NOCOUNT ON;
        UPDATE Cliente SET EsEmpleado = 0 WHERE IdCliente = @IdCliente;
        SELECT @@ROWCOUNT;
    END'
END
GO

-- trae nombre y apellido de una persona por dni (para precargar datos al vincular empleado como cliente)
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_ObtenerPersonaPorDni]') AND type in (N'P', N'PC'))
BEGIN
    EXEC sys.sp_executesql N'
    CREATE PROCEDURE SP_ObtenerPersonaPorDni
        @Dni VARCHAR(20)
    AS
    BEGIN
        SET NOCOUNT ON;
        SELECT IdPersona, Nombre, Apellido, DNI, Telefono, Email
        FROM Persona
        WHERE DNI = @Dni;
    END'
END
...
-- CREATE OR ALTER asegura que no salte error si ya existe,
-- y corrige cualquier versión anterior con esta estructura directamente.
